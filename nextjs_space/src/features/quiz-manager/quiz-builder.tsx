...
export function QuizBuilder({ initial, quizId }: { initial?: any; quizId?: string }) {
  ...
  function setOption(i: number, j: number, v: string) {
    setQuestions(qs =>
      qs.map((q, idx) =>
        idx === i
          ? { ...q, options: q.options.map((o, k) => k === j ? v : o) }
          : q
      )
    )
  }
  ...
  async function generateWithAI() {
    ...
    try {
      ...
      if (mode === "ai") {
        setQuestions(newQs.length ? newQs : [emptyQ()])
      } else {
        setQuestions(qs => [...qs.filter(q => q.text), ...newQs])
      }
  ...
  async function save() {
    if (!name || questions.length === 0) {
      toast.error("Completa nombre y al menos una pregunta")
      return
    }
    ...
    <div className="space-y-6">
      ...
      {(mode === "ai" || mode === "mixed") && (
        <div className="bg-primary/5 p-6 rounded-lg shadow-sm">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="font-display font-semibold">Generar con IA (Gemma 4)</h3>
          </div>

          <div className="grid md:grid-cols-[1fr,120px,auto] gap-3 items-end">
            <div className="space-y-2">
              <Label>Tema o contenido</Label>
              <Input
                value={aiTopic}
            ...
            <Button onClick={generateWithAI} disabled={busy}>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Button variant="outline" size="sm" onClick={() => setQuestions(qs => [...qs, emptyQ()])}>
        ...
        {questions.map((q, i) => (
          <div key={i} className="bg-card p-5 rounded-lg shadow-sm space-y-3">
            <div className="flex items-center justify-between gap-3">
              ...
              <Button size="sm" variant="ghost" onClick={() => setQuestions(qs => qs.filter((_, k) => k !== i))}>
            ...
            <p className="text-xs text-muted-foreground">
              Marca el círculo de las respuestas correctas. {q.type === "multiple" ? "Puedes marcar varias." : "Una sola."}
            </p>
          </div>
        ))}
      </div>
