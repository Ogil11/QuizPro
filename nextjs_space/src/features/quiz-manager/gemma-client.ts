...
function normalizeDifficulty(value: string): (typeof DIFFICULTIES)[number] {
  if (DIFFICULTIES.includes(value as any)) return value as (typeof DIFFICULTIES)[number]
...
`
  ...
  return `Eres un disenador instruccional experto. Genera ${count} preguntas de quiz en ESPANOL sobre el tema: "${topic}".
...
function validateQuestion(
  ...
  const errors: string[] = []
  if (!allowedTypes.includes(question.type)) errors.push(`tipo no permitido: ${question.type}`)
  if (!question.text || question.text.length < 12) errors.push("texto de pregunta demasiado corto")
  if (!question.explanation || question.explanation.length < 20) errors.push("explicacion ausente o demasiado breve")

  const questionKey = uniqueKey(question.text)
  if (seenQuestions.has(questionKey)) errors.push("pregunta duplicada o repetida")

  const optionKeys = question.options.map(uniqueKey)
  if (new Set(optionKeys).size !== question.options.length) errors.push("opciones duplicadas dentro de la pregunta")
...
export async function generateQuestions(
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS && accepted.length < safeCount; attempt += 1) {
    ...
    try {
      ...
      const { accepted: newQuestions, errors } = collectValidQuestions(parsed.questions, safeTypes, accepted)
      ...
      feedback = `${lastError}\nDevuelve solo el objeto JSON con "questions" y sin texto adicional.`
    }
  }
